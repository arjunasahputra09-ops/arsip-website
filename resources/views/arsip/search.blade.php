...
<div class="max-w-6xl mx-auto bg-white p-8 rounded-lg shadow-lg">
    ...
    <form action="{{ route('arsip.search') }}" method="GET" class="mb-6">
        <div class="flex">
            <input type="text" name="q" class="w-full rounded-l-lg shadow-sm" placeholder="Cari berdasarkan Judul, Kode, atau Deskripsi..." value="{{ request('q') }}">
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-r-lg font-medium hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                Cari
            </button>
        </div>
    </form>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                @forelse ($arsips as $arsip)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $arsip->tanggal->format('d M Y') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            @if ($arsip->file)
                                <a href="{{ route('arsip.download', $arsip->id) }}" class="text-blue-600 hover:text-blue-900">Download</a>
                            @else
